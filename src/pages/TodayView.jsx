...
export default function TodayView({ user }) {
  const [view, setView] = useState('day') // 'week' or 'day'
  ...
  function goToDay(offset) {
    ...
    if (!wk.find(w => dateKey(w) === dateKey(d))) setWeekBase(d)
  }

  const isToday = (d) => dateKey(d) === dateKey(today)
  const isSelected = (d) => dateKey(d) === selectedKey

  // Count events per day for week view dots
  function eventsOnDay(date) {
    const key = dateKey(date)
    return events.filter(e => eventOnDate(e, key))
  }
    ...
    <div style={{ display:'flex', flexDirection:'column', minHeight:'100vh', fontFamily:'-apple-system,sans-serif', background:'#f0f3f7' }}>
            ...
            {weekDates.map((date, i) => {
              ...
              return (
                <button key={i} onClick={() => selectDay(date)} style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'6px 2px 10px', background: sel ? 'rgba(42,181,160,0.25)' : 'transparent', border:'none', borderRadius:'8px 8px 0 0', cursor:'pointer', borderBottom: sel ? '3px solid #2ab5a0' : '3px solid transparent' }}>
                  ...
                  <span style={{ fontSize:16, fontWeight: sel || tod ? '700' : '400', color: tod ? '#2ab5a0' : 'white', width:28, height:28, display:'flex', alignItems:'center', justifyContent:'center', borderRadius:'50%', background: sel && tod ? 'rgba(42,181,160,0.3)' : 'transparent' }}>{date.getDate()}</span>
      ...
      <div style={{ flex:1, padding:'16px 16px 100px', overflowY:'auto' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
          <div>
            <div style={{ fontSize:22, fontWeight:'700', color:'#042746' }}>
              {isToday(selectedDate) ? 'Today' : DAYS_FULL[selectedDate.getDay()]}
            </div>
        ...
        <button onClick={loadEvents} style={{ width:'100%', padding:12, background:'transparent', border:'1px solid rgba(26,43,74,0.15)', borderRadius:8, fontSize:13, color:'#6b7a8d', cursor:'pointer', marginTop:8 }}>
          ↻ Refresh
        </button>
      </div>
    </div>
